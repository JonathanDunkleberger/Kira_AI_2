...
interface ConversationContextType {
  ...
  // Streak temporarily unsupported; expose optional field for compatibility
...
export default function ConversationProvider({ children }: { children: React.ReactNode }) {
  ...
  const processAudioChunk = useCallback(async (_audioBlob: Blob) => {
    // HTTP voice pipeline removed; rely on WebSocket path only.
    console.warn('processAudioChunk called but HTTP pipeline is removed; ensure WS is connected.');
  }, []);
  ...
  const submitAudioChunk = useCallback(async (audio: Blob) => {
    try {
      ...
      // If WS is connected, stream via socket; otherwise, drop with hint
      if (connectionStatus === 'connected') {
        ...
        try { 
          ...
          console.error('WS send failed; HTTP pipeline removed', e); 
        }
      } else {
        console.warn('WS not connected; audio chunk ignored because HTTP pipeline was removed.');
      }
    } catch (e) {
